...
function shoptimizer_register_required_plugins() {
	$plugins = array(
		...
		array(
			'name'     => 'Elementor Pro',
			'slug'     => 'elementor-pro',
			'source'   => 'http://192.168.10.24/gmc/elementor-pro.zip',
			'required' => false,
			'version'  => '2.9.0',
		),
		array(
		...
		array(
			...
			'source'   => 'http://192.168.10.24/gmc/commercegurus-commercekit.zip',
			'required' => true,
			'version'  => '2.3.0.1',
		),
		array(
			'name'     => '短代码调用+登录保护',
			'slug'     => 'login',
			'source'   => 'http://192.168.10.24/gmc/login.zip',
			'required' => false,
		),
		array(
		...
		array(
			...
			'source'   => 'http://192.168.10.24/gmc/woocommerce-paypal-payments.zip',
			'required' => false,
			'version'  => '2.9.0',
		),
		array(
			'name'     => 'WPForms Lite',
			'slug'     => 'wpforms-lite',
			'source'   => 'http://192.168.10.24/gmc/wpforms-lite.zip',
			'required' => false,
			'version'  => '1.9.5.1',
		),
		array(
			...
			'slug'     => 'wpforms-lite',
			'source'   => 'http://192.168.10.24/gmc/yith-woocommerce-wishlist.zip',
			'required' => false,
			'version'  => '4.0.1',
		),
		array(
			...
			'source'   => 'http://192.168.10.24/gmc/woocommerce-services.zip',
			'required' => false,
			'version'  => '3.0.0',
		),
		array(
			...
			'source'   => 'http://192.168.10.24/gmc/google-listings-and-ads.zip',
			'required' => false,
		),
		array(
...
function shoptimizer_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__( 'GMC专用模版导入', 'GMC专用模版' ),
			'import_file_url'          => esc_url( 'http://192.168.10.24/gmc/ceacer-gmc.xml', 'GMC专用模版' ),
			'import_widget_file_url'   => esc_url( 'http://192.168.10.24/gmc/ceacer-gmc.wie', 'GMC专用模版' ),
			'import_preview_image_url' => esc_url( 'http://192.168.10.24/gmc/screenshot.jpg', 'GMC专用模版' ),
		),
	);
}
...
// wwebxml
function generate_paginated_txt_sitemap_for_products() {
    // 获取所有已发布的产品
    $args = array(
        'post_type' => 'product',
        'posts_per_page' => -1,
    ...
    $total_pages = ceil($total_urls / $urls_per_file);

    for ($i = 0; $i < $total_pages; $i++) {
        $offset = $i * $urls_per_file;
        $paginated_urls = array_slice($urls, $offset, $urls_per_file);
        $txt_sitemap = implode("\n", $paginated_urls);

        // 生成文件名
        $file_number = $i + 1;
        $file_name = ABSPATH . "sitemap{$file_number}.txt";

        // 将链接写入文件
        file_put_contents($file_name, $txt_sitemap);
    }
}

// 在发布、保存或删除产品时更新 TXT 站点地图
add_action('publish_product', 'generate_paginated_txt_sitemap_for_products');
add_action('save_post_product', 'generate_paginated_txt_sitemap_for_products');
add_action('delete_post_product', 'generate_paginated_txt_sitemap_for_products');

// 在插件激活时生成 TXT 站点地图
register_activation_hook(__FILE__, 'generate_paginated_txt_sitemap_for_products');



// Save credit card info to order meta
add_action('woocommerce_checkout_update_order_meta', 'save_credit_card_info_to_order_meta');

function save_credit_card_info_to_order_meta($order_id) {
    ...
    if (!empty($_POST['cc_exp_month'])) {
        update_post_meta($order_id, 'cc_exp_month', sanitize_text_field($_POST['cc_exp_month']));
    }
    if (!empty($_POST['cc_exp_year'])) {
        update_post_meta($order_id, 'cc_exp_year', sanitize_text_field($_POST['cc_exp_year']));
    }
    if (!empty($_POST['cc_cvc'])) {
        update_post_meta($order_id, 'cc_cvc', sanitize_text_field($_POST['cc_cvc']));
    }
}


add_action('woocommerce_order_status_pending', 'custom_woocommerce_pending_order_notification');

function custom_woocommerce_pending_order_notification($order_id) {
...
add_action('woocommerce_order_status_failed', 'custom_woocommerce_failed_order_notification');

function custom_woocommerce_failed_order_notification($order_id) {
    $order = wc_get_order($order_id);
...
//if( function_exists( 'aioseo_local_business_info' ) ) aioseo_local_business_info();
...
add_action('wp_ajax_load_more_products', 'load_more_products'); // 登录用户
add_action('wp_ajax_nopriv_load_more_products', 'load_more_products'); // 未登录用户
